...
import time
from nn_model20 import DQNEfficientNet
...
def obs2stateTensor(obs, show=False):
    ...
    state = state.view(1, 84, 84)  # (H, W) -> C, H, W
...
for episode in range(episodes):
    ...
    losses.append(np.mean(epLosses))
    # save target model
    if(episode%10==0):
    ...
    if(episode>20):
        print(f"averge loss:{np.mean(losses[-21:-1])}, average reward:{np.mean(total_rewards[-21:-1])}")
